...
- **New `HistoryView`, manager-only, reached from the gear icon.** Sits in `AreaListView`'s toolbar beside Staff. Opens as a `.sheet` containing a `NavigationStack` + `List` of past business days (most recent first), drill into FOH / BOH sections, then logs under each. The currently-open business day is included at the top.
- **One canonical grouping: business day → Area → logs.** No alternate groupings (by duty, by user, by Phase). Managers answering "what happened in BOH last Wednesday night" get a single, predictable path. Revisit if a real reporting need surfaces.
...
- **Delete is irreversible.** Manager-only, no undo toast, no recycle bin. Confirmed via a standard `.confirmationDialog` before the `modelContext.delete` call. Same destructive-action shape as `StaffManagementView`'s delete-user flow.
...
- **`checklist_appApp.swift`** — `seedOnLaunch` calls `LogRetention.logsToDelete(in:asOf:)` and `modelContext.delete` each result. Runs after the open-business-day seed, before the UI appears. No user-visible latency (single `FetchDescriptor` with a `timestamp < cutoff` predicate; an empty result is a no-op).
- **`ContentView.swift` (`AreaListView`)** — gear-icon menu gains a "History" entry alongside "Staff." Manager-only, sheet presentation.
- **New file `checklist-app/History/HistoryView.swift`** — `NavigationStack` + `List` of past business days (most recent first). Drill into FOH / BOH groups. Logs under each group show duty name, timestamp, completer name, note (if any), and an "edited" badge if `lastEditedBy != nil`. Swipe-left on a log row to delete (with confirmation). Tap a row to open `LogEditSheet`.
